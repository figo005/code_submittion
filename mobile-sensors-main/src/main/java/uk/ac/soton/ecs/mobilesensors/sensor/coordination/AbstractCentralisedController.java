...
public abstract class AbstractCentralisedController implements
	...
	protected final List<Integer> getSensorsLocationIDs() {
		...
		for (Sensor sensor : getSensors()) {
			locationIDs.add(sensor.getLocation().getID());
					}
		return locationIDs;
	}
	...
	public void setSimulation(Simulation simulation) {
		...
		if (!registeredWithSimulator) {
			...
			initialController();//add by Shaofei
		}
	}
	
	protected abstract void initialController();//add by Shaofei
	
	public final void deleteSensors() {
		log.info("Sensors has registered with the controller");
