...
public class AccessibilityGraphImpl extends
	...
	private Map<Integer, Location> riskVertices = new HashMap<Integer, Location>();
	
	
	private Map<Integer, Integer> riskStatesIni = new HashMap<Integer, Integer>();
	
	private Map<Integer, Integer> informationStatesIni = new HashMap<Integer, Integer>();
	
	private Map<Integer, double[]> riskBeliefsIni = new HashMap<Integer, double[]>();
	
	private Map<Integer, double[]> informationBeliefsIni = new HashMap<Integer, double[]>();
...
//	private  double markovInformationModel[][];
	...
	public Location addAccessibleLocation(double x, double y,int id, double riskProbability) {
		LocationImpl newLocation = new LocationImpl(x, y, this,id,riskProbability);
		addVertex(newLocation);
		return newLocation;
	}
	
	public  void addAccessibleRiskLocation(int index, Location location) {

		if (riskVertices == null) {
			riskVertices = new HashMap<Integer, Location>();
	}
		riskVertices.put(index, location);
	}
	
	
	public  void addAccessibleRiskStateAndBelief(int index, int riskState, double riskBelief[]) {

		if (riskStatesIni == null) {
			riskStatesIni = new HashMap<Integer, Integer>();
	
		}
		if (riskBeliefsIni == null) {
			riskBeliefsIni = new HashMap<Integer, double[]>();
		}
		riskStatesIni.put(index, riskState);
		riskBeliefsIni.put(index, riskBelief);
	}
	
	public  void addAccessibleInformationStateAndBelief(int index, int informationState, double[] informationBelief) {

		if (informationStatesIni == null) {
			informationStatesIni = new HashMap<Integer, Integer>();
	
		}
		if (informationBeliefsIni == null) {
			informationBeliefsIni = new HashMap<Integer, double[]>();
		}
		informationStatesIni.put(index, informationState);
		informationBeliefsIni.put(index, informationBelief);
	}


	public  void addAccessibleMarkovRiskModel(int locationsNumber, double markovRiskModelTemp1[][], double markovRiskModelTemp2[][]) {

		if (markovRiskModel.isEmpty()) {
			
//			markovRiskModel = markovRiskModelTemp;
			
			for(int i=0; i<locationsNumber;i++)
			{
				if((i%7)==0)
				{
					markovRiskModel.add(markovRiskModelTemp1);
				}
					
				else
				{
					markovRiskModel.add(markovRiskModelTemp2);
				}
	...
	public  void addAccessibleMarkovInformationModel(int locationsNumber, double markovInformationModelTemp1[][],double markovInformationModelTemp2[][]) {

		if (markovInformationModel.isEmpty()) {
			for(int i=0; i<locationsNumber;i++)
			{
				if((i%3)==0)
				{
					markovInformationModel.add(markovInformationModelTemp1);
				}
					
				else
				{
					markovInformationModel.add(markovInformationModelTemp2);
				}
	...
	public Collection<Location> getRiskLoactions() {
		return riskVertices.values();
	}
	...
	public Map<Integer, Integer> getRiskStatesIni() {
		return riskStatesIni;
	}
	public Map<Integer, Integer> getInformationStatesIni() {
		return informationStatesIni;
	}
	public Map<Integer, double[]> getRiskBeliefsIni() {
		return riskBeliefsIni;
	}
	public Map<Integer, double[]> getInformationBeliefsIni() {
		return informationBeliefsIni;
	}
	public ArrayList<double[][]> getMarkovRiskModel() {
		return markovRiskModel;
	}
	public ArrayList<double[][]> getMarkovInformationModel() {
		return markovInformationModel;
	}
