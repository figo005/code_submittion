...
public class ReachableStateSpaceGraphNew <S extends StateNew, A extends Move>{
	...
	public ReachableStateSpaceGraphNew(S initial, TransitionFunctionNew<S,A> function) {
	...
	public Set<S> extend(S initial) {
		...
		while (!newStates.isEmpty()) {
			for (S state : newStates) {
				if(state.getHealthyLevelBelief().getExpectHealthy()<0.1)
				{
